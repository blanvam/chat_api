# encoding: utf-8
require 'dora/protocol/bin_tree_node'
require 'zlib'

require 'dora/protocol/bin_tree_reader/peek'
require 'dora/protocol/bin_tree_reader/read'
...
module Dora
  module Protocol
    class BinTreeNodeReader < BinTreeNode
      include BinTreeReader::Peek
      include BinTreeReader::Read
      ...
      def next_tree_internal
        ...
        if (size % 2) == 1
          return Node::ProtocolNode.new(tag, attributes, nil, '')
        end
        token = read_int8
        if is_list_tag(token)
          return Node::ProtocolNode.new(tag, attributes, read_list(token), '')
        end
        case token
          when 252
            len = read_int8
            data = fill_array len
            Node::ProtocolNode.new(tag, attributes, nil, data)
          when 253
            len = read_int20
            data = fill_array len
            Node::ProtocolNode.new(tag, attributes, nil, data)
          when 254
            ...
            Node::ProtocolNode.new(tag, attributes, nil, data)
          when 255, 251
            return Node::ProtocolNode.new(tag, attributes, nil, read_packed8(token))
          else
            return Node::ProtocolNode.new(tag, attributes, nil, read_string(token))
        end
      end
