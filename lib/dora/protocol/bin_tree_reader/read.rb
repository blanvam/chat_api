    ...
    module BinTreeReader
      module Read
        ...
        def read_string(token)
          raise ChatAPIError.new("BinTreeNodeReader read_string: Invalid -1 token #{token}") if token == -1
        ...
        def read_list_size(token)
          ...
          raise ChatAPIError.new("Read List Size: invalid list size token #{token}")
        end
        ...
        def get_token(token)
          ...
          if ret.nil?
            token = read_int8
            ret = Dora::Protocol::TokenMap.get_token(token, sub_dict)[0]
            if ret.nil?
              raise ChatAPIError.new("BinTreeNodeReader get_token: Invalid token / length #{token}")
            end
          end
          ret
        end
        ...
        def get_token_double(n1, n2)
          pos = n2 + n1 * 256
          ret = Dora::Protocol::TokenMap.get_token(pos, true)[0]
