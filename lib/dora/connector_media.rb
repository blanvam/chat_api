...
require 'dora/protocol/uploader'
...
module Dora
  ...
  class ConnectorMedia < Connector
    include Images
    include Videos
    ...
    def profile_image(jid, file_path)
      ...
      picture = ProtocolNode.new('picture', {type: 'image'}, nil, image)
      preview = ProtocolNode.new('picture', {type: 'preview'}, nil, preview)
    ...
    def send_request_file_upload(b64hash, type, size, file_path, to, caption = '')
      ...
      media_node = ProtocolNode.new('media', {hash: b64hash, type: type, size: size})
