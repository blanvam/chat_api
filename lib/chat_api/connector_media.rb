...
module Dora
  ...
  class ConnectorMedia < ConnectorSender
    ...
    def profile_image(jid, file_path)
      ...
      picture = Dora::Protocol::ProtocolNode.new('picture', {type: 'image'}, nil, image)
      preview = Dora::Protocol::ProtocolNode.new('picture', {type: 'preview'}, nil, preview)
    ...
    def send_image(to, file_path, store_url_media = false, fsize = 0, f_hash = '', caption = '')
      if fsize == 0 || f_hash == ''
        allowed_extensions = %w(jpg jpeg gif png)
        size = 5 * 1024 * 1024
        send_check_and_send_media(file_path, size, to, 'image', allowed_extensions, store_url_media, caption)
      else
        send_request_file_upload(f_hash, 'image', fsize, file_path, to, caption);
      end
    end
    ...
    def send_check_and_send_media(file_path, max_size, to, type, allowed_extensions, store_url_media, caption = '')
      if get_media_file(file_path, max_size)
        if allowed_extensions.include?(@media_file_info[:file_extension])
    ...
    def get_media_file(file_path, max_size_bytes = 5242880)
      if valid_uri?(file_path)
        @media_file_info = {}
        @media_file_info[:url] = file_path

        media = open(file_path).read
        @media_file_info[:file_size] = media.length
        ...
        if @media_file_info[:file_size] < max_size_bytes
          ...
          @media_file_info[:file_mime_type] = get_mime(@media_file_info[:file_path])
    ...
    def send_request_file_upload(b64hash, type, size, file_path, to, caption = '')
      ...
      media_node = Dora::Protocol::ProtocolNode.new('media', {hash: b64hash, type: type, size: size})
      node = Dora::Protocol::IqNode.new(id, WHATSAPP_SERVER, 'set', 'w:m', [media_node])
    ...
    def process_upload_response(node, id)
      ...
      if duplicate.nil?
        #upload new file
        json = Dora::Protocol::Uploader.push_file(node, message_node, @media_file_info, id)
        if json.nil?
          #failed upload
          return false
        end
      ...
      media_node = Dora::Protocol::ProtocolNode.new('media', media_attributes, nil, icon)
      if to.is_a?(Array)
