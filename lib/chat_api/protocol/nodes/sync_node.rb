...
module Dora
  module Protocol
    class SyncNode < ProtocolNode
      ...
      def SyncNode.mode_context(sync_type)
        case sync_type
          when 0
            mode = 'full'
      ...
      def SyncNode.users(numbers, deleted_numbers)
        users = []
        numbers.length.times do | i |
          users << ProtocolNode.new('user', nil, nil, (numbers[i][0, 1] != '+') ? ('+' + numbers[i]) : (numbers[i]))
        end
        unless deleted_numbers.nil?
          deleted_numbers.length.times do | j |
            users << ProtocolNode.new('user', {jid: deleted_numbers[j].to_jid.to_s, type: 'delete'})
          end
        end
      end
